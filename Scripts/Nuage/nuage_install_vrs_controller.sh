...
echo "${bold}[*] Update Neutron${normal}"
...
echo "${bold}[*] Apply Nuage Plugin${normal}"
...
mkdir openstack-upgrade
tar xzvf nuage-openstack-upgrade-5.3.2-20.tar.gz -C openstack-upgrade/
cd openstack-upgrade
python generate_cms_id.py --config-file /etc/neutron/plugin.ini

echo "${bold}[*] Update Horizon${normal}"
...
echo "${bold}[*] Restart Services${normal}"
